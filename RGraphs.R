...
barplot(height = data,legend.text = rownames(data),
...
legend(x="topr",legend = rownames(data),fill = c("lightblue","forestgreen"),title = "Car Origin",bg = NULL,bty = "n") #remove legend box using bty="n"
...
#3.1.2 with percentage
frme <- as.data.frame(Type)
pie(x = Type,labels = paste(frme[,1],round(frme[,2]*100/sum(frme[,2])),"%"))

pie ( x = Type,col = c("lightblue","forestgreen","yellow","orange","blue"),
    labels = paste(frme[,1],round(frme[,2]*100/sum(frme[,2])),"%"),
...
data <- data.frame(table(Cars$Type)[table(Cars$Type)!=0])
colnames(data) <- c("Type","Frequency")
ggplot(data = data,aes(x ="",y = Frequency,fill=Type ))+geom_bar(stat = "identity")+ coord_polar("y",start=0)+
  scale_fill_manual("Type",values = c("Compact"="lightblue","Large"="forestgreen",
                                      "Midsize" = "yellow","Small" = "orange","Sporty"="blue","Van"="black"))
#3.2.2 ggpot pie chart with values 
ggplot(data = data,aes(x ="",y = Frequency,fill=Type ))+geom_bar(stat = "identity")+ coord_polar("y",start=0)+
  geom_label(aes(label =  round(Frequency/ sum(Frequency) * 100)), position = position_fill(vjust =  c(40,80,160,450,370)))+
...
plot(x = Cars$EngineSize,y = Cars$Price,col="blue",ylab = "Price",xlab = "Engine size")
abline(lm(Cars$Price~Cars$EngineSize),col="red",lwd=2,lty=2)
...
#6.0.0 Boxplot______________________________________________________________

boxplot(x = Cars$Price[Cars$Origin=="USA"])
min(x = Cars$Price[Cars$Origin=="USA"])
min(k)
k = Cars$Price[Cars$Origin=="USA"]
quantile(k)[2:4]
IQR(k)
k[which(k>quantile(k)[4]+(1.5*IQR(k)))]

quantile(k)[2]-(1.5*IQR(k))

boxplot(x = Cars$Price[Cars$Origin=="non-USA"])
boxplot(x = cbind(Cars$Price[Cars$Origin=="non-USA"],Cars$Price[Cars$Origin=="USA"]))

ggplot(data = Cars,aes(x=Origin,y=Price,colour=Origin))+geom_boxplot(outlier.colour = "red",outlier.shape = 1)


#7.0.0 Multiple plots in one Place___________________________________________
